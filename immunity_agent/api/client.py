    ...
    def upload_context(self, id, project, request, control_flow, response):
        ...
        response = requests.post(
            ...
            json={
                ...
                "control_flow": base64.b64encode(control_flow.encode("utf-8")).decode(
                    "utf-8"
                ),
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Сбой отправки данных о запросе {id}. Код ответа: {response.status_code}; Содержимое ответа: {response.text}"
            )
        return response
